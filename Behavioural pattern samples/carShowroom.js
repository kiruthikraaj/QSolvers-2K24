...
class PaymentGateway {
  ...
  loadingDots() {
    throw new Error("Method 'loadingDots()' must be implemented.");
  }
}
...
class User extends UserAbstract {
  ...
  viewCarCollection() {
    if (this.carCollection.length > 0) {
      console.log(`${this.name}'s Car Collection:`);
      this.carCollection.forEach((carDecorator, index) => {
        console.log(`${index + 1}. ${carDecorator.getCarInfo()}`);
        carDecorator.viewAddOns();
      });
    } else {
